...
function brounhall_appointment_validate_payload( $payload ) {
	...
	$allowed = array( 'name', 'phone', 'email', 'location', 'service', 'message', 'consent', 'website', 'startedAt', 'clientToken', 'wpNonce' );
	if ( array_diff( array_keys( $payload ), $allowed ) ) { return new WP_Error( 'brounhall_invalid_appointment', 'Invalid appointment request', array( 'status' => 400 ) ); }
	...
	if ( $website || true !== $consent || ! preg_match( '/^[a-f0-9-]{16,100}$/', $client_token ) || ! $started_at || ! preg_match( '/^[\p{L}\p{M} .\',-]{2,100}$/u', $name ) || ! preg_match( '/^\+?[0-9 ()-]{7,25}$/', $phone ) || ! is_email( $email ) || ! in_array( $location, brounhall_appointment_allowed_locations(), true ) || ! in_array( $service, brounhall_appointment_allowed_services(), true ) || strlen( (string) ( $payload['message'] ?? '' ) ) > 2000 || time() * 1000 - $started_at < 1000 || time() * 1000 - $started_at > 86400000 ) {
